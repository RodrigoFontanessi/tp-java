...
public class EmpleadoLogicDefaultImpl implements EmpleadoLogic{
    ...
    private BiPredicate<Double,Double> salarioValidoEmpresa = (mediaEmpresa,salEmpleado)-> (mediaEmpresa == 0.0) || (salEmpleado>(mediaEmpresa*0.75) && salEmpleado < (mediaEmpresa*1.5));//true;
...
//TODO  2.2 defina un predicado donde un salario es valido si la media del departamento es 0.0 o si el salario se encuentra entre el 0.8 y el 1.2 del salario medio del departamento
    private BiPredicate<Double,Double> salarioValidoDepto = (mediaDepto,salEmpleado)-> (mediaDepto == 0.0) || (salEmpleado>(mediaDepto*0.8) && salEmpleado < (mediaDepto*1.2));//true;
    ...
    public void asignarDepartamento(Integer idEmpleado, Integer idDepto) {
        ...
        Double salarioMedioDepto = depto.getEmpleados().stream().mapToDouble(Empleado::getSalarioHora).average().orElse(0.0);
